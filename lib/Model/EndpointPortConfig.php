...
class EndpointPortConfig implements ModelInterface, ArrayAccess
{
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['published_port'] = isset($data['published_port']) ? $data['published_port'] : null;
        $this->container['publish_mode'] = isset($data['publish_mode']) ? $data['publish_mode'] : 'ingress';
    }
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['publish_mode']) && !in_array($this->container['publish_mode'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value for 'publish_mode', must be one of '%s'",
                implode("', '", $allowedValues)
            );
        }
     ...
     * Sets publish_mode
     *
     * @param string $publish_mode The mode in which port is published.  <p><br /></p>  - \"ingress\" makes the target port accessible on on every node,   regardless of whether there is a task for the service running on   that node or not. - \"host\" bypasses the routing mesh and publish the port directly on   the swarm node where that service is running.
     *
     * @return $this
     */
    ...
    {
        $allowedValues = $this->getPublishModeAllowableValues();
        if (!is_null($publish_mode) && !in_array($publish_mode, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'publish_mode', must be one of '%s'",
                    implode("', '", $allowedValues)
